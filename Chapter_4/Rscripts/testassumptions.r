...
test_assumption <- function(distD_, radius_, quantiles_dates, run_name, dat = acss_dat, danger_dat = danger_all) {
  ...
  results_ <- calculateNewDataAnalysis(
    dat = clippeddat,
...
assumptions_df <- tibble(
  distD_ = c(rep(150, 6), 300, 450, 50, rep(150, 6)),
  radius_ = c(rep(2500, 9), 1000, 5000, rep(2500, 4)),
  quantiles = list(
    c(0.1, 0.90), c(0.20, 0.80),
    c(0.05, 0.95),
    c(0.025, 0.975),
    c(0.005, 0.995),
    c(0.0, 1.0),
