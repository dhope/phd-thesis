calculateNewDataAnalysis <- function(dat, covars, outlier = 1995, plotOut = FALSE, binIt = FALSE, binD = NULL, seed = 57954, species = NULL, lgonly = T, useArea = F, rad_ = 2500) {
  ...
  sumYear <- summarizeData(dat,
    seed = seed, average = T, binIt = binIt, transform = "None",
  ...
  AUC.for.Analysis <-
    ...
    left_join(dat %>% group_by(Year) %>% summarise(
      SD_Doy = sd(DayNum),
      CV = SD_Doy / mean(DayNum),
      CV_ubiased = CV * (1 + 1 / (4 * n()))
    ), by = "Year") %>%
  ...
  if (isTRUE(plotOut)) {
    ...
    dailySummaryDanger <- dat %>%
      group_by(DayNum, Year) %>%
      summarize(danger = mean(proportionDanger)) %>%
      ungroup() %>%
      tidyr::spread(Year, danger, fill = NA) %>%
      tidyr::gather("Year", "danger", 2:43) %>%
      mutate(Year = as.numeric(Year))
    ...
    heatmaps <-
      plot_grid(
        ...
        ggplot(data = dailySummaryDanger, aes(Year, DayNum), environment = environment()) + geom_tile(aes(fill = danger)) +
          labs(y = "Day of Year", fill = "Mean\nSite\nDanger") + scale_fill_continuous_tableau(),
        labels = "auto",
    ...
    corrplot.mixed(cor(select(
      AUC.for.Analysis, mean.danger, CV, Lat.st, Lon.st, LgSiteCount,
      n.sites, BoFfalc, nfalc, Index, Yr.st
    ))) # fiftyDay
...
prepareDat <- function(sesa.dat, danger.dat, datelimits = c(0.1, 0.9), species = NULL) {
  ...
  danger.PRISM <- danger.dat %>%
    rename(area_km = IntArea, proportionDanger = danger) %>%
    mutate(Lat_full = round(Lat, 5), Lon_full = round(Lon, 5)) %>%
    filter(!is.na(proportionDanger)) %>%
    mutate(
      HighDanger = area_km * proportionDanger * 1000^2,
  ...
  spreadData <- birdObservations %>%
    group_by(GlobalUniqueIdentifier) %>%
    mutate(ObsCount = sum(ObservationCount, na.rm = T)) %>%
    ungroup() %>%
    dplyr::select(-ObservationCount) %>%
    rename(ObservationCount = ObsCount) %>%
    distinct() %>%
    dplyr::select(
      YearCollected, MonthCollected, DayCollected, # Locality,
      Lat, Lon,
      SurveyAreaIdentifier, SamplingEventIdentifier, CommonName, ObservationCount
    ) %>%
    spread(CommonName, ObservationCount, fill = 0)

  allCounts <- bind_rows(spreadData, dplyr::select(
    zeroObservations, YearCollected, MonthCollected, DayCollected, Lat, Lon, # Locality,
  ...
  pooledCounts <- allCounts.Zeros %>%
    group_by(YearCollected, MonthCollected, DayCollected, Lat, Lon) %>%
    select(-SurveyAreaIdentifier, -SamplingEventIdentifier) %>%
    summarize_all(funs(sum)) %>%
    ungroup() # summarise_each

  allCounts.w.Danger <- left_join(pooledCounts, dangerPooled, by = c("Lat", "Lon"))


  newCountsforAnalysis <- allCounts.w.Danger %>% dplyr::select(
    YearCollected, MonthCollected, DayCollected, # SamplingEventIdentifier,
    Locality,
    # SurveyAreaIdentifier,
    ID, Lat, Lon, proportionDanger, IntCells, IntertidalArea, utm, area_km, HighDanger, SESA.Zeros
  )

  ACSSCounts <- newCountsforAnalysis %>%
  ...
  ACSS_South_Danger <-
    ...
    mutate(
      Chronology = paste0(Year, DayNum), Danger.cat = ifelse(proportionDanger > 0.3, "Low", "High"),
      YrWeek = paste0(Year, Week)
    ) %>%
    group_by(YrWeek) %>%
    mutate(
      total.birds = sum(SESA.Zeros), est.birds = total.birds / n(),
      diff.expected = SESA.Zeros - est.birds
    ) %>%
    group_by(Chronology, Lat, Lon) %>%
    ungroup() %>%
    filter(!is.na(proportionDanger) & proportionDanger != 0)

  noBirdsObsed <- ACSS_South_Danger %>%
    group_by(Site.ID) %>%
    summarize(sumB = sum(SESA.Zeros, na.rm = T), n = n()) %>%
    ungroup() %>%
    filter(sumB <= 0) # & n>2)
