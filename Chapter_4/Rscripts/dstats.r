...
calculateDstat <- function(yr, data, method = "trapezoid", useArea = FALSE) {
  analysisData <- data %>% filter(Year == yr) %>% mutate(
    cdiff = cpropBirds - expectedpropBirds,
  ...
  output.df <- data.frame(
    Year = yr, aucExp = aucExp, aucObs = aucObs, meanSiteD = meanSiteD, maxSafe, minSafe,
...
summarizeData <- function(data, seed,
  ...
  if (isTRUE(average)) { # Should always be taking the average
    if (identical(catalogueOnly, FALSE)) { # Using not only catalogue sites  Depreciated also I believe.
      stop("None catalogue function not relevent for total PRISM data depreciated")
  ...
  subSampleCounts <-
    ...
    group_by(Year) %>%
    mutate_(
      TotalSurveyed = paste0("sum(", transformations[[transform]][1], ", na.rm=T)"),
      TotalBirdsCounted = paste0("sum(", transformations[[transform]][2], ", na.rm=T)")
    ) %>%
    ungroup() %>%
    rename(
      propDanger = proportionDanger,
      Area = IntertidalArea,
    ...
    ungroup() %>%
    mutate_(
      Area = transformations[[transform]][3],
      Birds = transformations[[transform]][4]
    ) # %>% mutate(Area = Area - AreaDanger) ## CHanged to test for only safe area

  ## Take each site, determine the proportion of all area surveyed in the year and the prop of all birds.
  # Then for each year, arrange by descending danger (All danger to all safe)
  if (isTRUE(binIt)) {
    dangerBins <- seq(0, 1, binSize)
    ...
    summarizedCount <-
      subSampleCounts %>%
      mutate(
        propDanger.bin = ifelse(rep(binSize < 0.001, nrow(.)), propDanger,
      ...
      group_by_(
        "Year.bin",
        paste(ifelse(length(dangerBins) <= length(areaBins),
          "propDanger.bin", "Area.bin"
        ))
      ) %>%
      mutate(n.sites = n()) %>%
      ...
      ungroup() %>%
      mutate(Year = Year.bin, Area = Area.bin, propDanger = propDanger.bin)
  } else {
    summarizedCount <-
      subSampleCounts %>%
  ...
  if (isTRUE(useArea)) {
    summarizedCountByYear <- summarizedCount %>%
      group_by(Year) %>%
      arrange(pArea) %>%
      # arrange(desc(propDanger))%>% # By arranging it this way, it makes the D stat less than when expected above
      # observed
      mutate(
        cpropBirds = cumsum(propBirds), # Take the cumulative sum of birds counted

        expectedpropBirds = cumsum(propArea), # Take the cumulative sum of area counted
        cArea = cumsum(Area),
        cBirds = cumsum(Birds),
        propSafety = 1 - propDanger
      ) %>%
      ungroup()
  } else {
    summarizedCountByYear <- summarizedCount %>%
      group_by(Year) %>%
      # arrange(desc(propArea)) %>%
      arrange(desc(propDanger)) %>% # By arranging it this way, it makes the D stat less than when expected above
      # observed
      mutate(
        cpropBirds = cumsum(propBirds), # Take the cumulative sum of birds counted
...
calculateDstats <- function(data, seed,
  ...
  summarizedCountByYear <- summarizeData(data, seed,
    ...
    useMice,
    modelType,
    binIt = binIt, binSize = binSize, yrbinSize = yrbinSize,
  ...
  if (isTRUE(plotOut)) {
    ...
    if (nrow(filter(runDF, !is.na(aucRatio))) >= 3) {
      aucPlot <- aucPlot + geom_smooth(method = "lm", se = F, alpha = 0.3)
    }
  }
  if (calcTrends) {
    if (modelType == "lm") {
...
boot.fun <- function(data, i, dataTransform) {
  df <- data[i, ] %>% group_by(Year) %>% mutate(
    lg =
      ifelse("45.72_-64.65" %in% Site.ID | "45.81_-64.5" %in% Site.ID, 0, 1)
  ) %>% ungroup()
  if (sum(df$lg) > 8) {
    output <- calculateDstats(data = df, plotOut = F, transform = dataTransform, average = T, seed = 565465)
    if (nrow(output) != length(unique(data$Year))) {
      return(rep(NA, length(unique(data$Year))))
    }
    else {
      return(output$aucRatio)
    }
  } else {
    return(rep(NA, length(unique(data$Year))))
  }
...
extract.ci <- function(boot.output, i, Print = F) {
  ...
  out.df <- data.frame(
    aucRatio = as.numeric(boot.output$t0[i]),
  ...
  if (Print) {
    plot(boot.output, index = i, main = paste(1974 + i))
  }
  return(out.df)
}
