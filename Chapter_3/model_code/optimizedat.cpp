...
int find_ideal(int r,double delta_var, int survey_total, double survey_p, int date, string variable)
{ 	string forFile = "/scratch/dhope/noU/optimizedValues" + variable + std::to_string(date)+"_"+std::to_string(r) + ".txt";
	 FILE *simfile;
	    ...
	    fprintf(simfile, "var\tdate\toptvar\toptN\n");
	    fclose(simfile);
...
/// Set up here, still need some variables
    ...
    fprintf(simfile, "%s\t%d\t%f\t%d\t%d\t%d\t%f\t%d\n", variable.c_str(), date,(double)modify_variable(variable,1, "return", date),finalNumbers, p,k, pLarge, totalBirds);
    fclose(simfile);
...
{	int rank, numprocs;
	...
	for(int r=0; r<10; r++){
		...
		{	
			...
			{find_ideal(r,1, totalBirds[row], pLarge[row], date_d[row],"popsize");
					number_birds[0]=baselineN;
					number_birds[1]=baselineN;} scenarioRun++;

			if ((scenarioRun % numprocs) == rank){		
					find_ideal(r,1, totalBirds[row], pLarge[row], date_d[row],"f_arrival_mod");
					f_arrival_mod = baselineArrival;}scenarioRun++;

			if ((scenarioRun % numprocs) == rank)
				{
					find_ideal(r,0.01, totalBirds[row], pLarge[row], date_d[row],"flywayFood");
					flywayFood=baselineFood;
				}scenarioRun++;

			if ((scenarioRun % numprocs) == rank)
				{
					find_ideal(r,0.01, totalBirds[row], pLarge[row], date_d[row],"flywayPredation");
