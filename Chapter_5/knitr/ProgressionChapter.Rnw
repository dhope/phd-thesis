...
kilometers, and both conditions encountered en route, and priorities of individuals can affect the timing of
migration. I used data from spring migration surveys of western sandpipers (\textit{Calidris
mauri}) and Pacific dunlins (\textit{Calidris alpina pacifica)} along the Pacific Flyway of
...
Survey counts were compiled from 6 sites on the northern Pacific Flyway (Figure 1, Table 1), from 5
...
small estuary one of the most productive in southern Puget Sound (Evenson and Buchanan 1997). In recent
...
flyway population) and 200,000-250,000 dunlins (30\%-50\% of flyway population; \citep*{drever_monitoring_2014}).
...
River Delta and Kachemak Bay (Iverson1996, Warnock1998, Warnock2004c). These patterns
...
failing, or less than 7 survey dates in a year; Supplementary Table S1). I determined confidence intervals
...
than northward along the northern portion of the flyway in Alaska (Table 1). For example, Copper River Delta lies
...
(see Figure 6 in the Supplementary Material Appendix A). For western sandpipers, 8 site years either failed
...
are shown in Supplementary Table S1. After outlier removal, the mixed effects models for both species
...
There is no consistent trend in local warming or cooling across the sites (Supplementary Figure S1). Across all years,
...
\subsection{Simulation Analysis: Effects of Behavior on Estimates of Peak Passage Dates}
...
distribution \citep{Limpert2001} with a mean value of length of stay based on the literature \citep{Warnock1998,Bishop2004a,Warnock2004}. The model was developed in Python 3.4 using NumPy \citep{Eric2001,VanderWalt2011}. The full Overview, Design concepts and Details (ODD) description \citep{GrimmVolkerandRailsback2005,Grimm2010} of the model is available in Supplementary Material Appendix B of this paper.
...
of overwintering area, age or sex (Warnock1998, Bishop2004a). To understand how presence of
...
and had a mean length of stay of 3 days shorter than the baseline strategy (Supplementary Appendix B). The proportion
...
knowledge of when migrants begin to arrive. I examined the impact this assumption had on the first analyzed survey
...
Estimated peak passage dates ({\textmu}) varied by species, year, and site (Supplementary Material Table
S1). 

\subsubsection{Western sandpipers.} 
The mean peak passage date ranged from 28 April (scaled to non-leap
years; day of year 118; Kennedy Creek) to 10 May (day of year 130; Kachemak Bay; Table 2).  The full
model that included the interaction between distance to breeding grounds and year had moderate
support from the data (\textit{wi}= 0.47) when compared to models without the
interaction, or without any fixed effects (Table 3a). The model without the interannual trend had almost equal
support (\textit{wi} = 0.37). 
...
Kennedy Creek) to 9 May (day of year 129; Kachemak Bay and Hartney Bay; Table 2). For the trend analysis, the
full model with a year and distance to the breeding grounds received strong support from the
data (\textit{wi}= 0.97; Table 3b; Figure 2; Figure 3C).  
...
southern sites. None of the effects had confidence intervals that overlapped zero (Figure 2). The interannual
...
without temperature (Table 3a). The model that received the most support from the data included variables from the
...
between northern and southern sites had confidence intervals that overlapped zero (Figure 2). 


Mean minimum temperature had different impacts on northern and southern sites (Figure 3B). An increase in
...
passage dates between northern and southern sites (Figure 3A). 


After accounting for the impact of temperature, the divergent interannual trends between northern and
southern sites remained (Figure 2). The 2 most southern sites were estimated to shift 0.08 and 0.09 days
...
distances to the breeding grounds alone (Figure 4). Hartney Bay appears to have peaks that occur \textasciitilde2 days later
...
(Table 3b; \textit{wi }=0.58). Though the second and third supported
model included the divergent temporal pattern, they also include the temperature effect and the divergent temperature
effect respectively (Table 3b). The random effect of site in the dunlin model showed greater variance than in
...
dunlins at Hartney Bay and the geographically adjacent Copper River Delta was even larger than for western sandpipers, with peaks separated by \textasciitilde5 days (Figure 3C).

\subsection{Simulation Analysis: Effects of Behavior on Estimates of Peak Passage}
...
day shifted peak passage date later by one-half day (0.50 {\textpm} 0.02 day; Figure 5A). I found no interactive
...
increased and the estimate shifted slightly earlier, although only by 0.15 day (Figure 5D).
...
population adopting the alternative strategy increased, the peak passage date shifted later (Figure 5B). A complete
...
Figure 5C), while when the site population was 50\% wintering residents, peak passage was only 2.69 days earlier
...
took an average of 9.1 {\textpm} 5.9 days (mean {\textpm} SD; n = 8) to make the 2,140 km journey to the Copper River Delta, AK \citep{Warnock2004c}. For western sandpipers radio-tagged in spring 1995 and 1996, the journey was estimated to take an average of 5.1 {\textpm} 2.2 days (Warnock1998, Bishop2004a,Warnock2004c). Using the fixed-effects from my models, I predicted the differences in dates of peak passage between these
...
\subsection{Behavioral Mechanisms for Observed Trends in Peak Passage Dates}
...
1992 (\textit{n = } 5; Iverson1996) and was 2.2 {\textpm} 1.2 days in 1995/96
 (\textit{n = } 25; Warnock1998), the model expectation of a 3 day reduction in
...
on the breeding grounds can vary widely between years \citep{Niehaus2006,Kwon2016}, there would
...
which is slower than that observed in individual tracking studies of western sandpipers, along the flyway (Iverson1996, Bishop2004a), but within the range of migration speeds observed across species \citep{Schmaljohann2017a}.
...
towards the breeding grounds after shorter length of stays than in colder years. Any behavioral shift could occur
...
assessing the health and behavior of migratory populations. Projects such as the Migratory Shorebird Project \citep{PointBlueConservationScience2014} are vital to standardizing protocols across a flyway and ensuring data from sites across a flyway can be brought together for more comprehensive analyses.
...
\hline
\end{tabular}
\caption{Geographic position of stopover sites and their distance to areas within the breeding range. Latitude (N) and longitude
...
\hline
\end{tabular}
\caption{Average timing and survey effort at 5 stopover sites on the Pacific Flyway listed north to south. Estimated peak passage
...
\end{tabular}
\caption{Support for models predicting peak passage date of migrating western sandpipers on northward migration; n=56 observations.
...
\end{tabular}
\caption{Support for models predicting peak passage date of
migrating dunlin on northward migration; n=44 observations. Model
descriptions are related to the full mixed model described in the text.
Models either used the distance from each site to the breeding area (D)
or the latitude of the stopover site (L). All model include a random
...
<<fig1-prog, out.width='0.8\\textwidth', fig.cap="Map of northeastern coast of the Pacific Ocean from Washington to Alaska. Sites used in this study included Kennedy Creek, Washington; Roberts Bank, British Columbia; Tofino, British Columbia; Copper River Delta, Alaska; and Kachemak Bay, Alaska.",fig.align="center">>=
