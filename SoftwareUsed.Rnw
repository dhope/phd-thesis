...
I wrote this thesis in \LaTeX  using the ``knitr'' program to build the figures within the thesis documents. The raw files are available on GitHub (\url{https://github.com/dhope/phd-thesis}) and within the SFU Research Data Repository. Anonymized survey results will also be available in the repository.
